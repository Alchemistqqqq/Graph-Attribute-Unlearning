...
from core.data.construct_embedding.random_walk import generate_random_walk_embeddings
def setup_logging():
    logging.basicConfig(
        level=logging.INFO,
...
def run(args):
    ...
    print("Generating structural embeddings for unlearn nodes...")
    ul_mask = ul_graph.ndata["unlearn_mask"]

    num_classes = int(graph.ndata['label'].max().item()) + 1
    struct_emb = generate_random_walk_embeddings(ul_graph,
                                                      walk_length=20,
                                                      walks_per_node=10,
                                                      embedding_dim=num_classes)
    full_node_struct_emb = torch.zeros(graph.num_nodes(), num_classes, device=struct_emb.device)
    full_node_struct_emb[ul_mask] = struct_emb[ul_mask] 
    ul_graph = ul_graph.to(args.device)  
    ul_graph.ndata['struct_feat'] = full_node_struct_emb
    print("Structural embeddings generated with shape:", full_node_struct_emb.shape)

    #-----------------------------------

    #nodes_to_zero = ul_mask == 1 
    #ul_graph.ndata['feat'][nodes_to_zero] = 0
