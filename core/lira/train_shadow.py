...
def train_shadow(args, model_number):
    ...
    keep_bool, sub_train_mask = Gen_keep(args, graph, model_number)
    ...
    train_loader = DataLoader(graph=graph,
                              ...
                              indices=torch.where(sub_train_mask == True)[0],
    ...
    test_loader = DataLoader(graph=graph,
                             batch_size=args.batch_size,
                             indices=torch.where(sub_train_mask == False)[0],
                             graph_sampler=sampler,
                             device=torch.device(args.device),
                             shuffle=True,
                             drop_last=True,
    ...
    print(f"Training finished for model {model_number}. Test accuracy: {test_acc[0]}")
